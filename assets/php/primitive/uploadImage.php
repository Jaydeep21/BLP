...
function uploadImages($Pid, $Images, $count = 0, $tag = '') {
    $nameNumber = $count;
    ...
    do {
        ...
        $nameNumber += 1;
        $imageRename = 'P' . $Pid . '_' . $date . '_' . $nameNumber;
...
<body>
    <form method="POST" id="imup" class="imup" action="#" enctype="multipart/form-data">
...
// if (isset($_POST['submit'])) {
    ...
    // if (!empty($Images['name'][0])) {
      ...
      // for( $count=0 ; $count < $total ; $count++ ) {
        ...
        // $tmpFilePath = $Images['tmp_name'];
    // if (!empty($Images['name'][0])) {
    //     foreach ($_FILES["images"]["error"] as $count => $error) {
    //         if ($error == UPLOAD_ERR_OK) {
    //             $tmp_name = $_FILES["images"]["tmp_name"][$count];
    //             if (!$tmp_name) continue;

    //             uploadImages(3, $Images, $count, $tag);
    //         }
    //         else {
    //             echo "Error  : " . $error;
    //         }
